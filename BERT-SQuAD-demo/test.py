...
def get_relevant_doc(vectorizer, query, sp_matrix, k=1):
    """
    참고 : vocab에 없는 이상한 단어로 query하는 경우 assertion 발생 ex) 땔뙇?w
    """
    ...
    return doc_scores[:k+3], doc_ids[:k+3]
...
def open_domain_qa(query, corpus, vectorizer, model, tokenizer, sp_matrix, k=1):
    ...
    for i in range(len(doc_id)):
        results.append([doc_id[i],doc_scores[i], get_answer_from_context(corpus[doc_id[i]], query, model, tokenizer)])
    return results
...
def parse_args():
    ...
    parser.add_argument('--model_name_or_path', default = "sillon/huggingface-tutorial", type=str, help='Path to pretrained model or model identifier from huggingface.co/models')
...
def inference(data):
    results = open_domain_qa(data, corpus, vectorizer, model, tokenizer, sp_matrix, k=args.topk)
...
if __name__ == "__main__":
    while True:
        ...
        results = inference(data) #id, scores, answer
        for i in range(len(results)):
            print('-'* 20 ,i,"번째 score 문서",'-'* 20 )
            print("score: ",results[i][1])
            print()
            print("context: ",results[i][0])
            print()
            print("quary:",data )
            print()
            print("answer:",results[i][2])
            print()
